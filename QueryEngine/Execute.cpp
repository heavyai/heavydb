...
ResultRows Executor::executeSelectPlan(const Planner::Plan* plan,
  ...
  if (dynamic_cast<const Planner::Scan*>(plan) || dynamic_cast<const Planner::AggPlan*>(plan)) {
    ...
    if (limit || offset) {
      ...
      auto rows = executeAggScanPlan(plan,
    ...
    return executeAggScanPlan(plan,
                              limit,
                              hoist_literals,
                              device_type,
                              opt_level,
...
std::pair<llvm::Value*, llvm::Value*> Executor::colByteStream(const int col_id,
                                                              const bool fetch_column,
                                                              const bool hoist_literals) {
...
llvm::Value* Executor::codegenCmp(const SQLOps optype,
  ...
  if (lhs_ti.is_integer() || lhs_ti.is_decimal() || lhs_ti.is_time() || lhs_ti.is_boolean() || lhs_ti.is_string()) {
    ...
    return not_null ? cgen_state_->ir_builder_.CreateICmp(llvm_icmp_pred(optype), lhs_lvs.front(), rhs_lvs.front())
  ...
  if (lhs_ti.get_type() == kFLOAT || lhs_ti.get_type() == kDOUBLE) {
    ...
    return not_null ? cgen_state_->ir_builder_.CreateFCmp(llvm_fcmp_pred(optype), lhs_lvs.front(), rhs_lvs.front())
...
llvm::Value* Executor::codegenArith(const Analyzer::BinOper* bin_oper, const bool hoist_literals) {
  ...
  if (lhs_type.is_fp()) {
    ...
    switch (optype) {
      case kMINUS:
        return not_null ? cgen_state_->ir_builder_.CreateFSub(lhs_lv, rhs_lv)
                        : cgen_state_->emitCall("sub_" + fp_typename + "_nullable", {lhs_lv, rhs_lv, fp_null});
      case kPLUS:
        return not_null ? cgen_state_->ir_builder_.CreateFAdd(lhs_lv, rhs_lv)
                        : cgen_state_->emitCall("add_" + fp_typename + "_nullable", {lhs_lv, rhs_lv, fp_null});
      case kMULTIPLY:
        return not_null ? cgen_state_->ir_builder_.CreateFMul(lhs_lv, rhs_lv)
...
ResultRows Executor::executeResultPlan(const Planner::Result* result_plan,
  ...
  *error_code = 0;
  ...
  launch_query_cpu_code(compilation_result.native_functions,
                        ...
                        error_code);
  CHECK(!*error_code);
  return query_exe_context->groupBufferToResults(0, target_exprs, 0, false);
}
...
ResultRows Executor::executeAggScanPlan(const Planner::Plan* plan,
                                        const int64_t limit,
                                        const bool hoist_literals,
  ...
  *error_code = 0;
  ...
  if (!max_groups_buffer_entry_guess) {
    ...
    auto it = std::max_element(fragments.begin(), fragments.end(), [](const FragmentInfo& f1, const FragmentInfo& f2) {
      return f1.numTuples < f2.numTuples;
    });
  ...
  const bool frag_vec_was_empty{all_fragment_results.empty()};
  if (agg_plan) {
    const bool output_columnar =
        compilation_result_cpu.native_functions.empty() ? compilation_result_gpu.output_columnar : false;
    try {
      return collectAllDeviceResults(all_fragment_results,
                                     ...
                                     row_set_mem_owner,
                                     output_columnar);
    } catch (SpeculativeTopFailed&) {
      // Optimistic reduction for top has failed, redo the result rows.
      CHECK(frag_vec_was_empty);
      decltype(all_fragment_results)().swap(all_fragment_results);
      return collectAllDeviceResults(all_fragment_results,
                                     plan,
                                     query_mem_desc,
                                     {gpu_sort_info.sort_plan, 0},
                                     device_type,
                                     query_contexts,
                                     row_set_mem_owner,
                                     output_columnar);
...
Executor::CompilationResult Executor::compilePlan(const Planner::Plan* plan,
  ...
  std::vector<llvm::Value*> col_heads;
  std::tie(cgen_state_->row_func_, col_heads) = create_row_function(scan_cols.size(),
...
!0 = metadata !{void (i8***,
  ...
  for (int device_id = 0; device_id < cuda_mgr->getDeviceCount(); ++device_id) {
    ...
    auto gpu_context =
        new GpuCompilationContext(ptx, func_name, gpu_rt_path.string(), device_id, cuda_mgr, blockSize());
