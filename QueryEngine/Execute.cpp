  ...
  *error_code = 0;
    ...
    try {
      compilation_result_cpu =
          compilePlan(plan,
                      ...
                      row_set_mem_owner,
                      max_groups_buffer_entry_guess,
                      render_allocator ? render_small_groups_buffer_entry_count_ : small_groups_buffer_entry_count_,
                      scan_limit,
                      sort_plan,
  ...
  auto dispatch = [this,
    ...
    for (const auto frag_id : outer_it->second) {
      const auto& outer_fragment = query_infos.front().fragments[frag_id];
    ...
    }
    {
      ...
      if (!device_results.definitelyHasNoRows()) {
        all_fragment_results.emplace_back(device_results, frag_ids.begin()->second);
      }
    }
    if (device_type == ExecutorDeviceType::Hybrid) {
  ...
  if (agg_plan) {
    ...
    try {
      return collectAllDeviceResults(
          all_fragment_results, plan, query_mem_desc, device_type, query_contexts, row_set_mem_owner, output_columnar);
  ...
  for (const auto& query_exe_context : query_contexts) {
    ...
    all_fragment_results.emplace_back(
        query_exe_context->getRowSet(
            get_agg_target_exprs(plan), query_mem_desc, device_type == ExecutorDeviceType::Hybrid),
        std::vector<size_t>{});
  }
...
std::vector<void*> Executor::optimizeAndCodegenGPU(llvm::Function* query_func,
  ...
  fill_options(option_keys, option_values, blockSize());
  ...
  checkCudaErrors(cuLinkAddData(link_state,
                                CU_JIT_INPUT_PTX,
                                static_cast<void*>(const_cast<char*>(ptx_nvptx.c_str())),
                                ptx_nvptx.length() + 1,
                                0,
                                num_options,
                                &option_keys[0],
