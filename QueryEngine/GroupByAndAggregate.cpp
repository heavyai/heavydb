...
QueryExecutionContext::QueryExecutionContext(
  ...
  const auto group_buffers_count =
      query_mem_desc_.group_col_widths.empty() ? 1 : num_buffers_;
...
bool countDescriptorsLogicallyEmpty(
...
void QueryExecutionContext::initColumnPerRow(const QueryMemoryDescriptor& query_mem_desc,
  ...
  for (size_t col_idx = 0; col_idx < query_mem_desc.agg_col_widths.size();
    ...
    if (!bm_sz || query_mem_desc.group_col_widths.empty()) {
      ...
      CHECK_EQ(static_cast<size_t>(query_mem_desc.agg_col_widths[col_idx].compact),
               sizeof(int64_t));
      init_val =
...
std::vector<ssize_t> QueryExecutionContext::allocateCountDistinctBuffers(
  ...
  for (size_t target_idx = 0, agg_col_idx = 0;
    ...
    if (is_distinct_target(agg_info)) {
      ...
      CHECK_EQ(static_cast<size_t>(query_mem_desc_.agg_col_widths[agg_col_idx].actual),
               sizeof(int64_t));
      CHECK_LT(target_idx, query_mem_desc_.count_distinct_descriptors_.size());
      const auto& count_distinct_desc =
          query_mem_desc_.count_distinct_descriptors_[target_idx];
...
std::vector<CUdeviceptr> QueryExecutionContext::prepareKernelParams(
  ...
  if (is_group_by && !output_columnar_) {
    ...
    auto cmpt_val_buff =
        compact_init_vals(cmpt_sz, init_agg_vals, query_mem_desc_.agg_col_widths);
...
std::pair<CUdeviceptr, CUdeviceptr> QueryExecutionContext::prepareTopNHeapsDevBuffer(
  ...
  auto dev_ptr =
      alloc_gpu_mem(data_mgr, thread_count * sizeof(CUdeviceptr), device_id, nullptr);
  copy_to_gpu(
      data_mgr, dev_ptr, &dev_buffers[0], thread_count * sizeof(CUdeviceptr), device_id);
  ...
  data_mgr->cudaMgr_->setDeviceMem(
...
GpuQueryMemory QueryExecutionContext::prepareGroupByDevBuffer(
  ...
  if (query_mem_desc_.lazyInitGroups(ExecutorDeviceType::GPU)) {
    ...
    if (output_columnar_) {
      ...
      col_widths_dev_ptr =
          alloc_gpu_mem(data_mgr, col_count * sizeof(int8_t), device_id, nullptr);
      copy_to_gpu(data_mgr,
                  col_widths_dev_ptr,
    ...
    if (!render_allocator) {
      if (use_streaming_top_n(ra_exe_unit, query_mem_desc_)) {
        ...
        CHECK_EQ(rows_copy.size(),
                 static_cast<size_t>(query_mem_desc_.entry_count *
...
std::vector<int64_t*> QueryExecutionContext::launchCpuCode(
  ...
  if (is_group_by) {
    cmpt_val_buff =
        compact_init_vals(align_to_int64(query_mem_desc_.getColsSize()) / sizeof(int64_t),
                          init_agg_vals,
                          query_mem_desc_.agg_col_widths);
  }
  ...
  if (use_streaming_top_n(ra_exe_unit, query_mem_desc_)) {
    ...
    const auto rows_copy = streaming_top_n::get_rows_copy_from_heaps(
        ...
        1);
    CHECK_EQ(rows_copy.size(),
             query_mem_desc_.entry_count * query_mem_desc_.getRowSize());
    memcpy(group_by_buffers_[0], &rows_copy[0], rows_copy.size());
  }
...
size_t QueryMemoryDescriptor::getConsistColOffInBytes(const size_t bin,
  ...
  return (keyless_hash ? 0 : sizeof(int64_t) * entry_count) +
         (col_idx * entry_count + bin) * agg_col_widths[0].compact;
}
...
size_t QueryMemoryDescriptor::getBufferSizeBytes(
  ...
  size_t total_bytes{0};
  if (output_columnar) {
    total_bytes = sizeof(int64_t) * group_col_widths.size() * entry_count +
                  getTotalBytesOfColumnarBuffers(agg_col_widths);
  } else {
...
int64_t GroupByAndAggregate::getBucketedCardinality(
...
GroupByAndAggregate::GroupByAndAggregate(
  ...
  while (true) {
    ...
    query_mem_desc_.sort_on_gpu_ = sort_on_gpu_hint &&
                                   query_mem_desc_.canOutputColumnar() &&
                                   !query_mem_desc_.keyless_hash;
    query_mem_desc_.is_sort_plan =
        !ra_exe_unit.sort_info.order_entries.empty() && !query_mem_desc_.sort_on_gpu_;
    ...
    if (query_mem_desc_.sortOnGpu() &&
        (query_mem_desc_.getBufferSizeBytes(device_type_) +
         align_to_int64(query_mem_desc_.entry_count * sizeof(int32_t))) >
            2 * 1024 * 1024 * 1024L) {
...
void GroupByAndAggregate::initQueryMemoryDescriptor(
  ...
  const auto min_byte_width =
      pick_target_compact_width(ra_exe_unit_, query_infos_, crt_min_byte_width);
  ...
  if (!is_group_by) {
    ...
    query_mem_desc_ = {executor_,
                       allow_multifrag,
                       ra_exe_unit_.estimator ? GroupByColRangeType::Estimator
  ...
  // For timestamp operations, adjust tollerance as integer overflows for nanosecs
  // precision.
  ...
  switch (col_range_info.hash_type_) {
    case GroupByColRangeType::OneColKnownRange: {
      ...
      const auto redirected_targets =
          redirect_exprs(ra_exe_unit_.target_exprs, ra_exe_unit_.input_col_descs);
      const auto keyless_info =
          getKeylessInfo(get_exprs_not_owned(redirected_targets), is_group_by);
      bool keyless = (!sort_on_gpu_hint || !many_entries(col_range_info.max,
                                                         col_range_info.min,
                                                         col_range_info.bucket)) &&
      ...
      bool shared_mem_for_group_by =
          g_enable_smem_group_by && keyless && keyless_info.shared_mem_support &&
      ...
      bool interleaved_bins = !shared_mem_for_group_by && keyless &&
                              (bin_count <= interleaved_max_threshold) &&
                              countDescriptorsLogicallyEmpty(count_distinct_descriptors);
      std::vector<ssize_t> target_group_by_indices;
      if (must_use_baseline_sort) {
        target_group_by_indices = target_expr_group_by_indices(ra_exe_unit_.groupby_exprs,
                                                               ra_exe_unit_.target_exprs);
        agg_col_widths.clear();
        for (auto wid :
             get_col_byte_widths(ra_exe_unit_.target_exprs, target_group_by_indices)) {
      ...
      if (shared_mem_for_group_by && (query_mem_desc_.getRowSize() > sizeof(int64_t))) {
        // We should not use the current shared memory path if more than 8 bytes per group
        // is required
        query_mem_desc_.sharing =
            GroupByMemSharing::Shared;  // disable the new shared memory path
        query_mem_desc_.interleaved_bins_on_gpu =
            keyless && (bin_count <= interleaved_max_threshold) &&
            countDescriptorsLogicallyEmpty(count_distinct_descriptors);
      }
      return;
    }
    case GroupByColRangeType::OneColGuessedRange: {
      ...
      query_mem_desc_ = {executor_,
                         ...
                         {},
                         max_groups_buffer_entry_count,
                         doRender ? render_info->render_small_groups_buffer_entry_count
                                  : small_groups_buffer_entry_count,
                         col_range_info.min,
                         col_range_info.max,
                         0,
    ...
    case GroupByColRangeType::MultiCol: {
      CHECK(!render_info || !render_info->isPotentialInSituRender());
      const auto target_group_by_indices = target_expr_group_by_indices(
          ra_exe_unit_.groupby_exprs, ra_exe_unit_.target_exprs);
      agg_col_widths.clear();
      for (auto wid :
           get_col_byte_widths(ra_exe_unit_.target_exprs, target_group_by_indices)) {
      ...
      const auto entries_per_shard =
          shard_count ? (max_groups_buffer_entry_count + shard_count - 1) / shard_count
                      : max_groups_buffer_entry_count;
      query_mem_desc_ = {executor_,
                         allow_multifrag,
                         col_range_info.hash_type_,
    ...
    case GroupByColRangeType::Projection: {
      CHECK(!must_use_baseline_sort);
      size_t group_slots = ra_exe_unit_.scan_limit
                               ? static_cast<size_t>(ra_exe_unit_.scan_limit)
                               : max_groups_buffer_entry_count;
      ...
      if (use_streaming_top_n(ra_exe_unit_, query_mem_desc_)) {
        query_mem_desc_.entry_count =
            ra_exe_unit_.sort_info.offset + ra_exe_unit_.sort_info.limit;
      }
...
CountDistinctDescriptors GroupByAndAggregate::initCountDistinctDescriptors() {
  ...
  for (const auto target_expr : ra_exe_unit_.target_exprs) {
    ...
    if (is_distinct_target(agg_info)) {
      ...
      GroupByAndAggregate::ColRangeInfo no_range_info{
          GroupByColRangeType::OneColGuessedRange, 0, 0, 0, false};
      auto arg_range_info =
...
GroupByAndAggregate::KeylessInfo GroupByAndAggregate::getKeylessInfo(
  ...
  for (const auto target_expr : target_expr_list) {
    ...
    // TODO(Saman): should be eventually removed, once I make sure what data types can be
    // used in this shared memory setting.
...
// TODO(Saman): this function is temporary and all these limitations should eventually be
// removed.
...
bool QueryMemoryDescriptor::usesCachedContext() const {
  return allow_multifrag && (usesGetGroupValueFast() ||
...
bool QueryMemoryDescriptor::threadsShareMemory() const {
  return sharing == GroupByMemSharing::Shared ||
         sharing == GroupByMemSharing::SharedForKeylessOneColumnKnownRange;
}
...
size_t QueryMemoryDescriptor::sharedMemBytes(const ExecutorDeviceType device_type) const {
  ...
  if (sharing == GroupByMemSharing::SharedForKeylessOneColumnKnownRange) {
    ...
    size_t shared_mem_size =
        (/*bin_count=*/entry_count + 1) * sizeof(int64_t);  // one extra for NULL values
...
llvm::Value* GroupByAndAggregate::codegenOutputSlot(llvm::Value* groups_buffer,
  ...
  if (use_streaming_top_n(ra_exe_unit_, query_mem_desc_)) {
    ...
    const auto chosen_bytes =
        static_cast<size_t>(query_mem_desc_.agg_col_widths[target_idx].compact);
...
std::tuple<llvm::Value*, llvm::Value*> GroupByAndAggregate::codegenGroupBy(
  ...
  switch (query_mem_desc_.hash_type) {
    ...
    case GroupByColRangeType::MultiColPerfectHash: {
      auto key_size_lv =
          LL_INT(static_cast<int32_t>(query_mem_desc_.group_col_widths.size()));
...
bool GroupByAndAggregate::codegenAggCalls(
  ...
  for (size_t target_idx = 0; target_idx < ra_exe_unit_.target_exprs.size();
    ...
    if ((executor_->plan_state_->isLazyFetchColumn(target_expr) || !is_group_by) &&
        static_cast<size_t>(query_mem_desc_.agg_col_widths[agg_out_off].compact) <
            sizeof(int64_t)) {
    ...
    if (agg_info.sql_type.is_geometry()) {
      ...
      // array. Ensure that the target values generated match the number of agg functions
      // before continuing
    ...
    for (const auto& agg_base_name : agg_fn_names) {
      if (agg_info.is_distinct && arg_expr->get_type_info().is_array()) {
        CHECK_EQ(static_cast<size_t>(query_mem_desc_.agg_col_widths[agg_out_off].actual),
                 sizeof(int64_t));
...
std::vector<llvm::Value*> GroupByAndAggregate::codegenAggArg(
  ...
  if (target_expr) {
    ...
    if (target_ti.is_geometry() &&
      ...
      size_t ctr = 0;
      for (const auto& target_lv : target_lvs) {
        // TODO(adb): consider adding a utility to sqltypes so we can get the types of the
        // physical coords cols based on the sqltype (e.g. TINYINT for col 0, INT for col
        // 1 for pols / mpolys, etc). Hardcoding for now. first array is the coords array
        // (TINYINT). Subsequent arrays are regular INT.
