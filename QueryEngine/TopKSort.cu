...
void collect_order_entry_column(thrust::device_ptr<K>& d_oe_col_buffer,
  ...
  thrust::for_each(thrust::cuda::par(allocator).on(cuda_stream),
...
void sort_indices_by_key(thrust::device_ptr<I> d_idx_first,
                         ...
                         CUstream cuda_stream) {
  if (desc) {
    thrust::sort_by_key(thrust::cuda::par(allocator).on(cuda_stream),
                        d_key_buffer,
    ...
    switch (layout.col_bytes) {
      case 4: {
        ...
        collect_order_entry_column(d_oe_buffer,
                                   ...
                                   cuda_stream);
        sort_indices_by_key(
            d_idx_first, idx_count, d_oe_buffer, oe.is_desc, allocator, cuda_stream);
        break;
      }
      case 8: {
...
std::vector<int8_t> pop_n_rows_from_merged_heaps_gpu(
  ...
  thrust::sequence(thrust::cuda::par(thrust_allocator).on(cuda_stream),
                   d_indices,
                   d_indices + total_entry_count);
  checkCudaErrors(cuStreamSynchronize(cuda_stream));
  auto separator =
      (group_key_bytes == 4)
          ? thrust::partition(thrust::cuda::par(thrust_allocator).on(cuda_stream),
                              d_indices,
                              d_indices + total_entry_count,
  ...
  thrust::for_each(thrust::cuda::par(thrust_allocator).on(cuda_stream),
                   thrust::make_counting_iterator(size_t(0)),
  ...
  if (final_entry_count < n) {
    reset_keys_in_row_buffer(thrust::cuda::par(thrust_allocator).on(cuda_stream),
