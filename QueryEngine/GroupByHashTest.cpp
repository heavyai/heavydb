...
void init_groups(int64_t* groups_buffer,
  ...
  for (int32_t i = 0; i < groups_buffer_entry_qw_count; ++i) {
    groups_buffer[i] = (i % (key_qw_count + 1) < key_qw_count)
                           ? EMPTY_KEY_64
                           : init_vals[(i - key_qw_count) % (key_qw_count + 1)];
  }
}
...
class GroupsBuffer {
 public:
  GroupsBuffer(const size_t groups_buffer_entry_count,
...
TEST(SetGetTest, OneKey) {
  ...
  GroupsBuffer gb(groups_buffer_entry_count, key_qw_count, 0);
  int64_t key = 31;
  auto gv1 = get_group_value(
      gb, groups_buffer_entry_count, &key, key_qw_count, sizeof(int64_t), row_size_quad);
  ASSERT_NE(gv1, nullptr);
  auto gv2 = get_group_value(
      gb, groups_buffer_entry_count, &key, key_qw_count, sizeof(int64_t), row_size_quad);
  ASSERT_EQ(gv1, gv2);
  int64_t val = 42;
  *gv2 = val;
  auto gv3 = get_group_value(
      gb, groups_buffer_entry_count, &key, key_qw_count, sizeof(int64_t), row_size_quad);
...
TEST(SetGetTest, OneKeyCollision) {
  ...
  int64_t key1 = 31;
  auto gv1 = get_group_value(
      gb, groups_buffer_entry_count, &key1, key_qw_count, sizeof(int64_t), row_size_quad);
  ASSERT_NE(gv1, nullptr);
  int64_t val1 = 32;
  *gv1 = val1;
  int64_t key2 = 41;
  auto gv2 = get_group_value(
      gb, groups_buffer_entry_count, &key2, key_qw_count, sizeof(int64_t), row_size_quad);
...
TEST(SetGetTest, MultiKeyRandom) {
  ...
  for (const auto key : keys) {
    auto gv = get_group_value(gb,
                              ...
                              row_size_quad);
    ASSERT_NE(gv, nullptr);
    ASSERT_EQ(*gv, std::accumulate(key.begin(), key.end(), 100, [](int64_t x, int64_t y) {
      return x + y;
    }));
  }
}
