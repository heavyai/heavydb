...
TEST(SetGetTest, OneKey) {
  ...
  auto gv2 = get_group_value(gb, groups_buffer_entry_count, &key, key_qw_count);
  ASSERT_EQ(gv1, gv2);
  int64_t val = 42;
  *gv2 = val;
  auto gv3 = get_group_value(gb, groups_buffer_entry_count, &key, key_qw_count);
...
TEST(SetGetTest, OneKeyCollision) {
  ...
  *gv1 = val1;
  int64_t key2 = 41;
  auto gv2 = get_group_value(gb, groups_buffer_entry_count, &key2, key_qw_count);
  ASSERT_NE(gv2, nullptr);
  int64_t val2 = 42;
  *gv2 = val2;
  gv1 = get_group_value(gb, groups_buffer_entry_count, &key1, key_qw_count);
  ASSERT_NE(gv1, nullptr);
  ASSERT_EQ(*gv1, val1);
  gv2 = get_group_value(gb, groups_buffer_entry_count, &key2, key_qw_count);
  ASSERT_NE(gv2, nullptr);
  ASSERT_EQ(*gv2, val2);
}
...
TEST(SetGetTest, OneKeyRandom) {
  ...
  for (int32_t i = 0; i < groups_buffer_entry_count; ++i) {
    ...
    keys.push_back(key);
    auto gv = get_group_value(gb, groups_buffer_entry_count, &key, key_qw_count);
...
TEST(SetGetTest, MultiKeyRandom) {
  ...
  for (int32_t i = 0; i < groups_buffer_entry_count; ++i) {
    ...
    keys.push_back(key);
    auto gv = get_group_value(gb, groups_buffer_entry_count, &key[0], key_qw_count);
    ASSERT_NE(gv, nullptr);
