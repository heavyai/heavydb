...
GpuGroupByBuffers create_dev_group_by_buffers(
  ...
  if (!render_allocator && (always_init_group_by_on_host ||
    ...
    cuda_allocator.copyToDevice(
        group_by_dev_buffers_mem, group_by_buffers[0], mem_size, device_id);
  }
  ...
  for (size_t i = 0; i < num_ptrs; i++) {
    group_by_dev_buffers[i] = group_by_dev_buffers_mem;
  }
...
void copy_group_by_buffers_from_gpu(Data_Namespace::DataMgr* data_mgr,
  ...
  CHECK_EQ(size_t(1), group_by_buffers.size());
  // if there's no index buffer existing at the beginning:
  if (!prepend_index_buffer) {
  ...
  // If there's an index buffer at the beginning (e.g., in columnar projections)
  const size_t index_buffer_sz{
      prepend_index_buffer ? query_mem_desc.getEntryCount() * sizeof(int64_t) : 0};

  const size_t buffer_size = groups_buffer_size + index_buffer_sz;
  copy_from_gpu(data_mgr,
                group_by_buffers[0],
