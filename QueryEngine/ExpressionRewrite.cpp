...
bool window_exprs_args_match(const Analyzer::WindowFunction* lhs,
                             const Analyzer::WindowFunction* rhs) {
  const auto& lhs_args = lhs->getArgs();
...
bool window_sum_and_count_match(const Analyzer::UOper* cast_sum_window_expr,
  ...
  return window_exprs_args_match(sum_window_expr, count_window_expr);
}
