...
class DeepCopyVisitor : public ScalarExprVisitor<std::shared_ptr<Analyzer::Expr>> {
 protected:
  ...
  RetType visitDotProduct(const Analyzer::DotProductExpr* dot_product) const override {
    return makeExpr<Analyzer::DotProductExpr>(visit(dot_product->get_arg1()),
                                              visit(dot_product->get_arg2()));
  }
  ...
  RetType visitGeoH3Oper(const Analyzer::GeoH3Oper* geo_expr) const override {
    std::vector<std::shared_ptr<Analyzer::Expr>> args0_copy;
    for (const auto& arg : geo_expr->getArgs0()) {
      args0_copy.push_back(visit(arg.get()));
    }
    const auto func_name = geo_expr->getFuncName();
    const auto& type_info = geo_expr->get_type_info();
    return makeExpr<Analyzer::GeoH3Oper>(func_name, type_info, args0_copy);
  }
  ...
  RetType visitFragmentIdAndOffset(
      const Analyzer::FragmentIdAndOffset* oper) const override {
    return makeExpr<Analyzer::FragmentIdAndOffset>(oper->getColVar());
  }
};
