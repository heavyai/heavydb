...
void QueryFragmentDescriptor::buildFragmentDeviceMap(const RelAlgExecutionUnit& ra_exe_unit,
  ...
  if (enable_multifrag_kernels) {
    buildMultifragKernelMap(ra_exe_unit, frag_offsets, device_count, device_type, enable_inner_join_fragment_skipping);
  } else {
...
void QueryFragmentDescriptor::buildFragmentPerKernelMap(const RelAlgExecutionUnit& ra_exe_unit,
  ...
  for (size_t i = 0; i < outer_fragments->size(); ++i) {
    ...
    const auto skip_frag =
        executor_->skipFragment(outer_table_desc, fragment, ra_exe_unit.simple_quals, frag_offsets, i);
    if (skip_frag.first) {
      continue;
    }
    rowid_lookup_key_ = std::max(rowid_lookup_key_, skip_frag.second);
    const int chosen_device_count = device_type == ExecutorDeviceType::CPU ? 1 : device_count;
    CHECK_GT(chosen_device_count, 0);
    const auto memory_level =
        device_type == ExecutorDeviceType::GPU ? Data_Namespace::GPU_LEVEL : Data_Namespace::CPU_LEVEL;
    ...
    for (size_t j = 0; j < ra_exe_unit.input_descs.size(); ++j) {
      const auto frag_ids = executor_->getTableFragmentIndices(
          ra_exe_unit, device_type, j, i, selected_tables_fragments_, executor_->getInnerTabIdToJoinCond());
...
void QueryFragmentDescriptor::buildMultifragKernelMap(const RelAlgExecutionUnit& ra_exe_unit,
  ...
  for (size_t outer_frag_id = 0; outer_frag_id < outer_fragments->size(); ++outer_frag_id) {
    ...
    auto skip_frag =
        executor_->skipFragment(outer_table_desc, fragment, ra_exe_unit.simple_quals, frag_offsets, outer_frag_id);
    if (enable_inner_join_fragment_skipping && (skip_frag == std::pair<bool, int64_t>(false, -1))) {
      skip_frag =
          executor_->skipFragmentInnerJoins(outer_table_desc, ra_exe_unit, fragment, frag_offsets, outer_frag_id);
    }
    if (skip_frag.first) {
      continue;
    }
    ...
    for (size_t j = 0; j < ra_exe_unit.input_descs.size(); ++j) {
      ...
      CHECK(table_frags_it != selected_tables_fragments_.end());
      const auto frag_ids = executor_->getTableFragmentIndices(
          ra_exe_unit, device_type, j, outer_frag_id, selected_tables_fragments_, inner_table_id_to_join_condition);
      if (fragments_per_device_[device_id].size() < j + 1) {
        fragments_per_device_[device_id].emplace_back(FragmentsPerTable{table_id, frag_ids});
      } else {
