...
#ifdef HAVE_CUDA
template <typename T>
void sort_on_gpu(T* val_buff, int32_t* idx_buff, const uint64_t entry_count, const bool desc) {
...
template <typename T>
void apply_permutation_on_gpu(T* val_buff, int32_t* idx_buff, const uint64_t entry_count, T* tmp_buff) {
  thrust::device_ptr<T> key_ptr(val_buff);
  thrust::device_ptr<int32_t> idx_ptr(idx_buff);
