...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_d_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ddd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(3+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=3; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dddd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(4+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=4; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ddddd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(5+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=5; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dddddd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(6+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=6; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ddddddd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(7+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=7; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dddddddd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(8+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=8; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ddddddddd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(9+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=9; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dddddddddd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(10+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=10; ++i) {
     const auto arg = translateScalarRex(rex_function->getOperand(i));
     args.push_back(arg);
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_f_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(3+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=3; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ffff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(4+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=4; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fffff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(5+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=5; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ffffff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(6+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=6; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fffffff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(7+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=7; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ffffffff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(8+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=8; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fffffffff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(9+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=9; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ffffffffff_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(10+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=10; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_l_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_lll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(3+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=3; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_llll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(4+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=4; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_lllll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(5+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=5; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_llllll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(6+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=6; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_lllllll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(7+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=7; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_llllllll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(8+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=8; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_lllllllll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(9+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=9; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_llllllllll_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(10+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=10; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_i_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_iii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(3+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=3; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_iiii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(4+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=4; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_iiiii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(5+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=5; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_iiiiii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(6+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=6; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_iiiiiii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(7+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=7; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_iiiiiiii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(8+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=8; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_iiiiiiiii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(9+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=9; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_iiiiiiiiii_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(10+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=10; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_d_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_f_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dd_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_df_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_df_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fd_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fd_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ff_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_d_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_l_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dd_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dl_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dl_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ld_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ld_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ll_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_d_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_i_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_dd_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_di_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_di_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_id_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_id_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ii_d(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_l_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_f_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ll_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_lf_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_lf_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fl_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fl_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ff_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_l_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_i_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ll_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_li_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_li_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_il_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_il_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ii_l(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_i_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_f_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(1+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=1; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ii_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_if_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_if_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fi_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_fi_f(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
std::shared_ptr<Analyzer::Expr> RelAlgTranslator::translatePyudf_ff_i(
     const RexFunctionOperator* rex_function) const {
   CHECK_EQ(rex_function->size(), size_t(2+1));
   std::vector<std::shared_ptr<Analyzer::Expr>> args;
   for(int i=0; i<=2; ++i) {
...
if (rex_function->getName() == std::string("PYUDF_D_D")) {
    return translatePyudf_d_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DD_D")) {
    return translatePyudf_dd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DDD_D")) {
    return translatePyudf_ddd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DDDD_D")) {
    return translatePyudf_dddd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DDDDD_D")) {
    return translatePyudf_ddddd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DDDDDD_D")) {
    return translatePyudf_dddddd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DDDDDDD_D")) {
    return translatePyudf_ddddddd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DDDDDDDD_D")) {
    return translatePyudf_dddddddd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DDDDDDDDD_D")) {
    return translatePyudf_ddddddddd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DDDDDDDDDD_D")) {
    return translatePyudf_dddddddddd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_F_F")) {
    return translatePyudf_f_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FF_F")) {
    return translatePyudf_ff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FFF_F")) {
    return translatePyudf_fff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FFFF_F")) {
    return translatePyudf_ffff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FFFFF_F")) {
    return translatePyudf_fffff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FFFFFF_F")) {
    return translatePyudf_ffffff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FFFFFFF_F")) {
    return translatePyudf_fffffff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FFFFFFFF_F")) {
    return translatePyudf_ffffffff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FFFFFFFFF_F")) {
    return translatePyudf_fffffffff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FFFFFFFFFF_F")) {
    return translatePyudf_ffffffffff_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_L_L")) {
    return translatePyudf_l_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LL_L")) {
    return translatePyudf_ll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LLL_L")) {
    return translatePyudf_lll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LLLL_L")) {
    return translatePyudf_llll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LLLLL_L")) {
    return translatePyudf_lllll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LLLLLL_L")) {
    return translatePyudf_llllll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LLLLLLL_L")) {
    return translatePyudf_lllllll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LLLLLLLL_L")) {
    return translatePyudf_llllllll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LLLLLLLLL_L")) {
    return translatePyudf_lllllllll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LLLLLLLLLL_L")) {
    return translatePyudf_llllllllll_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_I_I")) {
    return translatePyudf_i_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_II_I")) {
    return translatePyudf_ii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_III_I")) {
    return translatePyudf_iii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IIII_I")) {
    return translatePyudf_iiii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IIIII_I")) {
    return translatePyudf_iiiii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IIIIII_I")) {
    return translatePyudf_iiiiii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IIIIIII_I")) {
    return translatePyudf_iiiiiii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IIIIIIII_I")) {
    return translatePyudf_iiiiiiii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IIIIIIIII_I")) {
    return translatePyudf_iiiiiiiii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IIIIIIIIII_I")) {
    return translatePyudf_iiiiiiiiii_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_D_F")) {
    return translatePyudf_d_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_F_D")) {
    return translatePyudf_f_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DD_F")) {
    return translatePyudf_dd_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DF_D")) {
    return translatePyudf_df_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DF_F")) {
    return translatePyudf_df_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FD_D")) {
    return translatePyudf_fd_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FD_F")) {
    return translatePyudf_fd_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FF_D")) {
    return translatePyudf_ff_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_D_L")) {
    return translatePyudf_d_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_L_D")) {
    return translatePyudf_l_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DD_L")) {
    return translatePyudf_dd_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DL_D")) {
    return translatePyudf_dl_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DL_L")) {
    return translatePyudf_dl_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LD_D")) {
    return translatePyudf_ld_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LD_L")) {
    return translatePyudf_ld_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LL_D")) {
    return translatePyudf_ll_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_D_I")) {
    return translatePyudf_d_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_I_D")) {
    return translatePyudf_i_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DD_I")) {
    return translatePyudf_dd_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DI_D")) {
    return translatePyudf_di_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_DI_I")) {
    return translatePyudf_di_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_ID_D")) {
    return translatePyudf_id_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_ID_I")) {
    return translatePyudf_id_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_II_D")) {
    return translatePyudf_ii_d(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_L_F")) {
    return translatePyudf_l_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_F_L")) {
    return translatePyudf_f_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LL_F")) {
    return translatePyudf_ll_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LF_L")) {
    return translatePyudf_lf_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LF_F")) {
    return translatePyudf_lf_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FL_L")) {
    return translatePyudf_fl_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FL_F")) {
    return translatePyudf_fl_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FF_L")) {
    return translatePyudf_ff_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_L_I")) {
    return translatePyudf_l_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_I_L")) {
    return translatePyudf_i_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LL_I")) {
    return translatePyudf_ll_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LI_L")) {
    return translatePyudf_li_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_LI_I")) {
    return translatePyudf_li_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IL_L")) {
    return translatePyudf_il_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IL_I")) {
    return translatePyudf_il_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_II_L")) {
    return translatePyudf_ii_l(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_I_F")) {
    return translatePyudf_i_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_F_I")) {
    return translatePyudf_f_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_II_F")) {
    return translatePyudf_ii_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IF_I")) {
    return translatePyudf_if_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_IF_F")) {
    return translatePyudf_if_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FI_I")) {
    return translatePyudf_fi_i(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FI_F")) {
    return translatePyudf_fi_f(rex_function);
}

if (rex_function->getName() == std::string("PYUDF_FF_I")) {
    return translatePyudf_ff_i(rex_function);
}
