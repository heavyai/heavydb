 ...
 public:
  ...
  void visitNode(const RelAlgNode* node) const {
    ...
    if (auto join = const_cast<RelJoin*>(dynamic_cast<const RelJoin*>(node))) {
      CHECK_EQ(!!join->getConditionExpr(), !!join->getCondition());
      if (join->getConditionExpr()) {
        std::unique_ptr<const RexScalar> rex_cond(join->getAndReleaseCondition());
        RexVisitor::visit(rex_cond.get());
        auto cond = DeepCopyVisitor::visit(join->getConditionExpr());
        join->setCondition(rex_cond, std::move(cond));
      }
      return;
    }
...
void propagate_rex_input_renumber(
  ...
  while (!work_set.empty()) {
    ...
    if (auto project = dynamic_cast<RelProject*>(modified_node)) {
      ...
      for (size_t i = 0; i < project->size(); ++i) {
        new_scalar_exprs.push_back(renumber.visit(project->getProjectAt(i)));
        new_exprs.push_back(visitor.visit(project->getExpr(i).get()));
      }
      project->setExpressions(std::move(new_scalar_exprs), std::move(new_exprs));
      continue;
    }
    CHECK(false);
  }
...
void redirect_inputs_of(
  ...
  if (auto project = std::dynamic_pointer_cast<RelProject>(node)) {
    ...
    hdk::ir::ExprPtrVector new_exprs;
    for (size_t i = 0; i < project->size(); ++i) {
      new_scalar_exprs.push_back(redirector.visit(project->getProjectAt(i)));
      new_exprs.push_back(visitor.visit(project->getExpr(i).get()));
    }
    project->setExpressions(std::move(new_scalar_exprs), std::move(new_exprs));
    return;
...
void try_insert_coalesceable_proj(
  ...
  for (auto node : nodes) {
    ...
    auto project_owner = std::make_shared<RelProject>(
        std::move(scalar_exprs), std::move(exprs), fields, node);
...
sweep_dead_columns(
  ...
  for (auto node : nodes) {
    ...
    if (auto aggregate = std::dynamic_pointer_cast<RelAggregate>(node)) {
      ...
      auto old_exprs = project->getExprs();
      std::vector<std::unique_ptr<const RexScalar>> new_scalar_exprs;
      hdk::ir::ExprPtrVector new_exprs;
      std::vector<std::string> new_fields;
      for (size_t i = 0; i < old_scalar_exprs.size(); ++i) {
        if (old_live_outs.count(i)) {
          new_scalar_exprs.push_back(std::move(old_scalar_exprs[i]));
...
void propagate_input_renumbering(
  ...
  while (!work_set.empty()) {
    ...
    if (auto project = dynamic_cast<RelProject*>(node)) {
      auto old_scalar_exprs = project->getExpressionsAndRelease();
      std::vector<std::unique_ptr<const RexScalar>> new_scalar_exprs;
      for (auto& expr : old_scalar_exprs) {
        new_scalar_exprs.push_back(renumberer.visit(expr.get()));
      }
      hdk::ir::ExprPtrVector new_exprs;
      ...
      auto new_condition_expr = visitor.visit(join->getConditionExpr());
      join->setCondition(new_condition, std::move(new_condition_expr));
    ...
    if (join && join->getJoinType() == JoinType::INNER) {
      ...
      if (auto literal = dynamic_cast<const RexLiteral*>(join->getCondition())) {
        ...
        if (src_join == join) {
          ...
          auto filter_condition_expr = modified_filter->getConditionExprShared();
          ...
          join->setCondition(filter_condition, std::move(filter_condition_expr));
          continue;
        }
        ...
        if (join_conditions.size() == 1) {
          ...
          for (size_t i = 1; i < join_condition_exprs.size(); ++i) {
            auto rhs = visitor.visit(join_condition_exprs[i]);
            new_join_condition_expr = hdk::ir::makeExpr<hdk::ir::BinOper>(
                kBOOLEAN, kAND, kONE, new_join_condition_expr, rhs);
          }
          join->setCondition(new_join_condition, new_join_condition_expr);
        }

        SubConditionRemover orig_remover(join_conditions);
        auto new_filter_condition = orig_remover.visit(filter->getCondition());
        SubConditionRemoveVisitor remover(join_condition_exprs);
