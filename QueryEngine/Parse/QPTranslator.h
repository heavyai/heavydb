#ifndef AST_QUERY_PLAN_VISITOR_H
#define AST_QUERY_PLAN_VISITOR_H
...
public:
    ...
    ProductOp* formProductOp(class TableRefCommalist *vTRC, class TableRef *vTR) {
        if (!(vTRC)) return new ProductOp(new RelExpr(new Relation(vTR->tbl)), NULL);
        else return new ProductOp(new RelExpr(formProductOp(vTRC->trc, vTRC->vtr)), new RelExpr(new Relation(vTR->tbl)));
    }

    RelExpr* formProjection(class Selection *v) {
        if (v->selectAll == "*") return new RelExpr(ProjectOp())
        return new RelExpr(formProductOp(v->sec));
    }

    ProductOp* formProjectOp(class TableRefCommalist *vTRC, class TableRef *vTR) {
        if (!(vTRC)) return new ProductOp(new RelExpr(new Relation(vTR->tbl)), NULL);
 ...
 //       v->OAD->accept(*this);
        ...
        RelExpr* finishedProduct = formProdRelation(v->tblExp->fc->trc);
...
//        cout << "</SelectStatement>" << endl;
    ...
    /********************
    ADD SOMETHING THAT MAKES A REL_EXPR LIST FROM EACH PART OF THE SQLLIST 
    **************************/        
...
#endif // AST_QUERY_PLAN_VISITOR_H
