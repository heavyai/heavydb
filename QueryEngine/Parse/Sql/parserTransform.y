...
%token  OPT_LIMIT_CLAUSE PRODUCT PROJECT LIKE_PREDICATE LIKE ESCAPE OPT_ESCAPE
...
%type <nPtr> opt_limit_clause like_predicate opt_escape
...
predicate:
    ...
    |   like_predicate                          { $$ = opr(PREDICATE, 1, $1); }
    /*  |   test_for_null
    |   in_predicate
...
like_predicate:
        scalar_exp NOT LIKE atom opt_escape                     { $$ = opr(LIKE_PREDICATE, 5, $1, opr(NOT, 0), opr(LIKE, 0), $4, $5); }
    |   scalar_exp LIKE atom opt_escape                         { $$ = opr(LIKE_PREDICATE, 4, $1, opr(LIKE, 0), $3, $4); }
    ;

opt_escape:
        /* empty */                                             { $$ = opr(OPT_ESCAPE, 1, opr(EMPTY, 0)); }
    |   ESCAPE atom                                             { $$ = opr(OPT_ESCAPE, 2, opr(ESCAPE, 0), $2); }
    ;
