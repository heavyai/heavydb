...
extern "C" __device__ void init_group_by_buffer_gpu_impl(int64_t* groups_buffer,
  ...
  for (int32_t i = start; i < groups_buffer_entry_qw_count; i += step) {
    if (i % row_size_quad < key_qw_count) {
      groups_buffer[i] = EMPTY_KEY_64;
    } else {
      groups_buffer[i] = init_vals[(i % row_size_quad) - key_qw_count];
    }
  }
  __syncthreads();
}
...
__device__ int8_t* init_columnar_buffer(T* buffer_ptr,
                                        ...
                                        const int32_t start,
                                        const int32_t step) {
  for (int32_t i = 0; i < entry_count; i += step) {
...
__global__ void init_group_by_buffer_gpu_wrapper(int64_t* groups_buffer,
  ...
  init_group_by_buffer_gpu_impl(
      groups_buffer, init_vals, groups_buffer_entry_count, key_qw_count, row_size_quad, keyless, warp_size);
}
