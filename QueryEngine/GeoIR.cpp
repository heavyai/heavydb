...
std::vector<llvm::Value*> CodeGenerator::codegenGeoFunctionOperator(
  ...
  if (geo_func_oper->getName() == "ST_Perimeter" ||
    ...
    return {ret};
  } else if (geo_func_oper->getName() == "ST_Centroid") {
    const auto operand = geo_func_oper->getArg(0);
    const auto& arg_ti = operand->get_type_info();
    const uint32_t coords_elem_sz_bytes =
        arg_ti.get_compression() == kENCODING_GEOINT ? 4 : 8;
    ...
    auto elem_ty = llvm::Type::getDoubleTy(cgen_state_->context_);
    ...
    if (ret_ti.get_compression() == kENCODING_GEOINT) {
      auto elem_ty = llvm::Type::getInt32Ty(cgen_state_->context_);
      llvm::ArrayType* arr_type = llvm::ArrayType::get(elem_ty, 2);
    ...
    auto idx_lv = cgen_state_->llInt(0);
    ...
    if (ret_ti.get_compression() == kENCODING_GEOINT) {
      ...
      auto x_ptr = builder.CreateGEP(
          pt_local_storage_lv, {cgen_state_->llInt(0), cgen_state_->llInt(0)}, "x_ptr");
      auto x_lv = builder.CreateLoad(x_ptr);
      auto compressed_x_lv =
          cgen_state_->emitExternalCall("compress_x_coord_geoint",
                                        llvm::Type::getInt32Ty(cgen_state_->context_),
                                        {x_lv});
      auto compressed_x_ptr =
          builder.CreateGEP(pt_compressed_local_storage_lv,
                            {cgen_state_->llInt(0), cgen_state_->llInt(0)},
                            "compressed_x_ptr");
      builder.CreateStore(compressed_x_lv, compressed_x_ptr);

      auto y_ptr = builder.CreateGEP(
          pt_local_storage_lv, {cgen_state_->llInt(0), cgen_state_->llInt(1)}, "y_ptr");
      auto y_lv = builder.CreateLoad(y_ptr);
      auto compressed_y_lv =
          cgen_state_->emitExternalCall("compress_y_coord_geoint",
                                        llvm::Type::getInt32Ty(cgen_state_->context_),
                                        {y_lv});
      auto compressed_y_ptr =
          builder.CreateGEP(pt_compressed_local_storage_lv,
                            {cgen_state_->llInt(0), cgen_state_->llInt(1)},
                            "compressed_y_ptr");
    ...
    if (is_nullable) {
      ...
      ret_coords = nullcheck_codegen->finalize(ret_coords, ret_coords);
    }
    return {ret_coords,
            cgen_state_->llInt(ret_ti.get_compression() == kENCODING_GEOINT ? 8 : 16)};
  }
