...
__attribute__((__used__)) std::shared_ptr<LidarMetadata> get_metadata_for_file(
...
__attribute__((__used__)) std::pair<int64_t, std::vector<std::shared_ptr<LidarMetadata>>>
get_metadata_for_files(const std::vector<std::filesystem::path>& file_paths,
                       const std::string& out_srs) {
...
__attribute__((__used__)) bool keys_are_cached(
    const std::string& filename,
    const std::string& out_srs,
...
__attribute__((__used__)) void PdalFileMgr::openAndPrepareFile() {
  auto timer = DEBUG_TIMER(__func__);
  if (is_ready_) {
    return;
  }
...
__attribute__((__used__)) void PdalFileMgr::readData(double* x,
                                                     double* y,
                                                     double* z,
                                                     int32_t* intensity,
                                                     int8_t* return_num,
