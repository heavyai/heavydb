...
#ifdef HAVE_RENDERING
    if (gfx_context_) {
      static bool shaders_added = false;
      if (!shaders_added) {
        auto& shader_manager = gfx_context_->getShaderManager();
        auto& shader_library = shader_manager.getLibrary();
        QueryEngineShaders::ShaderLibraryAutoGenerated::add_all_to_library(
            [&](auto const& item) {
              shader_library.add(
      ...
      CHECK(device_group.size() > 0);
      // use first device only
      auto& device_context = gfx_context_->getDeviceContext(device_group[0]);
      // create an execution context
      gfx_execution_context_ = device_context.createCommandExecutionContext();
    }
#endif
  }
