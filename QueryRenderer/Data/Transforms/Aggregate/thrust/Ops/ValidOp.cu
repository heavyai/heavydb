...
struct ValidEval {
  struct is_key_notnull {
    const int64_t invalid_key;
    const T null_val;

    is_key_notnull(const int64_t invalid_key, const T null_val)
  ...
  struct is_not_null {
    ...
    is_not_null(T null_val) : null_val{null_val} {}
  ...
  static AggDataList eval(DataMgrThrustContext& execution_context,
    ...
    if (state.key_col) {
      ...
      val = ::thrust::count_if(
          ...
          zip_begin,
          zip_end,
          is_key_notnull(state.interop_buffer_info.invalid_key, null_val));
    } else {
      val = ::thrust::count_if(execution_context.getDevicePolicy(),
                               col->begin(),
                               col->end(),
                               is_not_null(null_val));
    }
...
AggDataList ThrustOpExecutorImpl<active_device_system>::executeValidOp(
    ThrustOpExecutionState state) {
  return ThrustOpExecutorUtils::runMultiTypeOp<ValidEval>(
