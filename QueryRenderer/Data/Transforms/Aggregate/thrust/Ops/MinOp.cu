...
struct MinimumEval {
  ...
  struct tupleCompare : public nullCompare {
  ...
  static AggDataList eval(DataMgrThrustContext& execution_context,
    ...
    if (state.key_col) {
      ...
      auto min_elem = ::thrust::min_element(
          execution_context.getDevicePolicy(),
          zip_begin,
          zip_end,
          tupleCompare(state.interop_buffer_info.invalid_key, null_val));
      if (min_elem == zip_end ||
          ::thrust::get<1>(*min_elem) == state.interop_buffer_info.invalid_key) {
        val = null_val;
      } else {
        val = ::thrust::get<0>(*min_elem);
      }
    } else {
      auto min_elem = ::thrust::min_element(execution_context.getDevicePolicy(),
                                            col->begin(),
                                            col->end(),
                                            nullCompare(null_val));

      // the < 0 check below is to handle the special case where all values are null. The
      // thrust implementation of min_element here sets an initial value of
      // std::numerical_limits<T>::max() with an iterator offset of -1. See
      // <thrust/system/detail/generic/extrema.inl>::min_element
      // If all the elements are null, the nullCompare() check will always return true,
      ...
      if (min_elem == col->end() || min_elem - col->begin() < 0) {
        val = null_val;
      } else {
        val = *min_elem;
      }
...
template <>
AggDataList MinimumEval<int64_t>::getReturnVal(const SQLTypeInfo& type_info,
                                               const int64_t& val) {
...
AggDataList ThrustOpExecutorImpl<active_device_system>::executeMinOp(
    ThrustOpExecutionState state) {
  return ThrustOpExecutorUtils::runMultiTypeOp<MinimumEval>(
