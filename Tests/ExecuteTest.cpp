...
TEST(Select, FilterAndGroupBy) {
  for (auto dt : { ExecutorDeviceType::CPU, ExecutorDeviceType::GPU }) {
    ...
    c("SELECT dec as key, COUNT(*) AS value FROM test GROUP BY key HAVING key IS NOT NULL ORDER BY key, value DESC LIMIT 12;", dt);
  }
}
...
TEST(Select, Case) {
  for (auto dt : { ExecutorDeviceType::CPU, ExecutorDeviceType::GPU }) {
    ...
    c("SELECT SUM(CASE WHEN x BETWEEN 6 AND 7 THEN 1.1 WHEN x BETWEEN 8 AND 9 THEN 2.2 ELSE 3.3 END) "
      "FROM test WHERE CASE WHEN y BETWEEN 44 AND 45 THEN 5.1 ELSE 3.9 END > 4;", dt);
    c("SELECT CASE WHEN x BETWEEN 1 AND 3 THEN 'oops 1' WHEN x BETWEEN 4 AND 6 THEN 'oops 2' ELSE real_str END from test;", dt);
    c("SELECT CASE WHEN x BETWEEN 1 AND 3 THEN 'oops 1' WHEN x BETWEEN 4 AND 6 THEN 'oops 2' ELSE str END from test;", dt);
    c("SELECT CASE WHEN x BETWEEN 1 AND 7 THEN 'seven' WHEN x BETWEEN 7 AND 10 THEN 'eight' ELSE 'ooops' end from test;", dt);
    c("SELECT CASE WHEN x BETWEEN 1 AND 7 THEN 'seven' WHEN x BETWEEN 7 AND 10 THEN real_str ELSE 'ooops' END AS g FROM test;", dt);
    c("SELECT CASE WHEN x BETWEEN 1 AND 7 THEN 'seven' WHEN x BETWEEN 7 AND 10 THEN str ELSE 'ooops' END FROM test;", dt);
    c("SELECT CASE WHEN x BETWEEN 1 AND 7 THEN 'seven' WHEN x BETWEEN 7 AND 10 THEN 'eight' ELSE 'ooops' END from test;", dt);
    c("SELECT CASE WHEN x BETWEEN 1 AND 7 THEN str WHEN x BETWEEN 7 AND 10 THEN 'eight' ELSE 'ooops' END AS g, COUNT(*) FROM test GROUP BY g ORDER BY g;", dt);
  }
}
...
void import_array_test(const std::string& table_name) {
  ...
  const auto col_descs = cat.getAllColumnMetadataForTable(td->tableId, false);
