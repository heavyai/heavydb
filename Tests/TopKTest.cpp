...
std::shared_ptr<ResultSet> run_multiple_agg(
    const string& query_str, const ExecutorDeviceType device_type, const bool allow_loop_joins)
{
    return run_multiple_agg(query_str, g_session, device_type, g_hoist_literals, allow_loop_joins);
}

std::shared_ptr<ResultSet> run_multiple_agg(
    const string& query_str, const ExecutorDeviceType device_type)
{
...
public:
    ...
    void compare_arrow_output(const std::string& query_string,
        const std::string& sqlite_query_string, const ExecutorDeviceType device_type)
    {
        const auto results
            = run_select_query(query_string, g_session, device_type, g_hoist_literals, true);
    ...
    void compare(const std::string& query_string, const std::string& sqlite_query_string,
    ...
    void compare_timstamp_approx(
        const std::string& query_string, const ExecutorDeviceType device_type)
    {
        const auto mapd_results = run_multiple_agg(query_string, device_type);
...
private:
    ...
    void compare_impl(const MapDResults* mapd_results, const std::string& sqlite_query_string,
        const ExecutorDeviceType device_type, bool timestamp_approx)
    {
                ...
                switch (mapd_type)
                {
                    ...
                    case kDATE:
                    {
                        ...
                        else
                        {
                            ...
                            const auto ref_val = connector_.getData<std::string>(row_idx, col_idx);
                            const auto end_str = strptime(ref_val.c_str(),
                            ...
                            if (end_str != nullptr)
                            {
                                ASSERT_EQ(0, *end_str);
                                ASSERT_EQ(
                                    ref_val.size(), static_cast<size_t>(end_str - ref_val.c_str()));
                            }
                            if (timestamp_approx)
                            {
...
int create_and_populate_tables()
{
    try
    {
        ...
        // Create a table
        const std::string create_test_table{"CREATE TABLE tdata (id SMALLINT, b BOOLEAN, i INT, bi BIGINT, n DECIMAL(10, 2), f FLOAT, t TEXT, tt TIME, d DATE, ts TIMESTAMP, vc VARCHAR(15));"};
        ...
        data_col_value_list.push_back("1, 't', 23, 2349923, 111.1, 1.1, 'SFO', '15:13:14', '1999-09-09', '2014-12-13 22:23:15', 'paris'");
        data_col_value_list.push_back("2, 'f', null, -973273, 7263.11, 87.1, null, '20:05:00', '2017-12-12', '2017-12-12 20:05:00', 'toronto'");
        data_col_value_list.push_back("3, 'f', 702, 87395, 333.5, null, 'YVR', '11:11:11', '2010-01-01', '2010-01-02 04:11:45', 'vancouver'");
        data_col_value_list.push_back("4, null, 864, 100001, null, 9.9, 'SJC', null, '2015-05-05', '2010-05-05 05:15:55', 'london'");
        data_col_value_list.push_back("5, 'f', 333, 112233, 99.9, 9.9, 'ABQ', '22:22:22', '2015-05-05', '2010-05-05 05:15:55', 'new york'");
        data_col_value_list.push_back("6, 't', -3, 18, 765.8, 2.2, 'YYZ', '00:00:01', null, '2009-01-08 12:13:14', null");
        data_col_value_list.push_back("7, 'f', -9873, 3789, 789.3, 4.7, 'DCA', '11:22:33', '2001-02-03', '2005-04-03 15:16:17', 'rio de janerio'");
        data_col_value_list.push_back("8, 't', 12, 4321, 83.9, 1.2, 'DXB', '21:20:10', null, '2007-12-01 23:22:21', 'dubai'");
        data_col_value_list.push_back("9, 't', 48, null, 83.9, 1.2, 'BWI', '09:08:07', '2001-09-11', null, 'washington'");
        data_col_value_list.push_back("10, 'f', 99, 777, 77.7, 7.7, 'LLBG', '07:07:07', '2017-07-07', '2017-07-07 07:07:07', 'Tel Aviv'");
...
TEST(Select, TopK_LIMIT_DescendSort) {
    ...
    SKIP_NO_GPU();
        ...
        c("SELECT bi FROM tdata ORDER BY bi DESC NULLS LAST LIMIT 5;", "SELECT bi FROM tdata ORDER BY bi DESC LIMIT 5;", dt);
        ...
        c("SELECT tt FROM tdata ORDER BY tt DESC NULLS LAST LIMIT 5;", "SELECT tt FROM tdata ORDER BY tt DESC LIMIT 5;", dt);
        c("SELECT ts FROM tdata ORDER BY ts DESC NULLS LAST LIMIT 5;", "SELECT ts FROM tdata ORDER BY ts DESC LIMIT 5;", dt);
...
TEST(Select, TopK_LIMIT_GreaterThan_TotalOfDataRows_DescendSort) {
    ...
    SKIP_NO_GPU();
        ...
        c("SELECT bi FROM tdata ORDER BY bi DESC NULLS LAST LIMIT 11;", "SELECT bi FROM tdata ORDER BY bi DESC LIMIT 11;", dt);
        ...
        c("SELECT tt FROM tdata ORDER BY tt DESC NULLS LAST LIMIT 11;", "SELECT tt FROM tdata ORDER BY tt DESC LIMIT 11;", dt);
        c("SELECT ts FROM tdata ORDER BY ts DESC NULLS LAST LIMIT 11;", "SELECT ts FROM tdata ORDER BY ts DESC LIMIT 11;", dt);
...
TEST(Select, TopK_LIMIT_OFFSET_TopHalf_AscendSort) {
    for (auto dt : {ExecutorDeviceType::CPU, ExecutorDeviceType::GPU}) {
    SKIP_NO_GPU();
        c("SELECT i FROM tdata ORDER BY i NULLS FIRST LIMIT 5 OFFSET 0;", "SELECT i FROM tdata ORDER BY i LIMIT 5 OFFSET 0;", dt);
        c("SELECT b FROM tdata ORDER BY b NULLS FIRST LIMIT 5 OFFSET 0;", "SELECT b FROM tdata ORDER BY b LIMIT 5 OFFSET 0;", dt);
        c("SELECT bi FROM tdata ORDER BY bi NULLS FIRST LIMIT 5 OFFSET 0;", "SELECT bi FROM tdata ORDER BY bi LIMIT 5 OFFSET 0;", dt);
        c("SELECT n FROM tdata ORDER BY n NULLS FIRST LIMIT 5 OFFSET 0;", "SELECT n FROM tdata ORDER BY n LIMIT 5 OFFSET 0;", dt);
        c("SELECT f FROM tdata ORDER BY f NULLS FIRST LIMIT 5 OFFSET 0;", "SELECT f FROM tdata ORDER BY f LIMIT 5 OFFSET 0;", dt);
        c("SELECT tt FROM tdata ORDER BY tt NULLS FIRST LIMIT 5 OFFSET 0;", "SELECT tt FROM tdata ORDER BY tt LIMIT 5 OFFSET 0;", dt);
        c("SELECT ts FROM tdata ORDER BY ts NULLS FIRST LIMIT 5 OFFSET 0;", "SELECT ts FROM tdata ORDER BY ts LIMIT 5 OFFSET 0;", dt);
        c("SELECT d FROM tdata ORDER BY d NULLS FIRST LIMIT 5 OFFSET 0;", "SELECT d FROM tdata ORDER BY d LIMIT 5 OFFSET 0;", dt);
    }
}
...
TEST(Select, TopK_LIMIT_OFFSET_TopHalf_DescendSort) {
    for (auto dt : {ExecutorDeviceType::CPU, ExecutorDeviceType::GPU}) {
    SKIP_NO_GPU();
        c("SELECT i FROM tdata ORDER BY i DESC NULLS LAST LIMIT 5 OFFSET 0;", "SELECT i FROM tdata ORDER BY i DESC LIMIT 5 OFFSET 0;", dt);
        c("SELECT b FROM tdata ORDER BY b DESC NULLS LAST LIMIT 5 OFFSET 0;", "SELECT b FROM tdata ORDER BY b DESC LIMIT 5 OFFSET 0;", dt);
        c("SELECT bi FROM tdata ORDER BY bi DESC NULLS LAST LIMIT 5 OFFSET 0;", "SELECT bi FROM tdata ORDER BY bi DESC LIMIT 5 OFFSET 0;", dt);
        c("SELECT n FROM tdata ORDER BY n DESC NULLS LAST LIMIT 5 OFFSET 0;", "SELECT n FROM tdata ORDER BY n DESC LIMIT 5 OFFSET 0;", dt);
        c("SELECT f FROM tdata ORDER BY f DESC NULLS LAST LIMIT 5 OFFSET 0;", "SELECT f FROM tdata ORDER BY f DESC LIMIT 5 OFFSET 0;", dt);
        c("SELECT tt FROM tdata ORDER BY tt DESC NULLS LAST LIMIT 5 OFFSET 0;", "SELECT tt FROM tdata ORDER BY tt DESC LIMIT 5 OFFSET 0;", dt);
        c("SELECT ts FROM tdata ORDER BY ts DESC NULLS LAST LIMIT 5 OFFSET 0;", "SELECT ts FROM tdata ORDER BY ts DESC LIMIT 5 OFFSET 0;", dt);
        c("SELECT d FROM tdata ORDER BY d DESC NULLS LAST LIMIT 5 OFFSET 0;", "SELECT d FROM tdata ORDER BY d DESC LIMIT 5 OFFSET 0;xxx", dt);
    }
}
...
TEST(Select, DISABLED_TopK_LIMIT_OFFSET_BottomHalf_AscendSort) {
    for (auto dt : {ExecutorDeviceType::CPU, ExecutorDeviceType::GPU}) {
    SKIP_NO_GPU();
        c("SELECT i FROM tdata ORDER BY i NULLS FIRST LIMIT 5 OFFSET 5;", "SELECT i FROM tdata ORDER BY i LIMIT 5 OFFSET 5;", dt);
        c("SELECT b FROM tdata ORDER BY b NULLS FIRST LIMIT 5 OFFSET 5;", "SELECT b FROM tdata ORDER BY b LIMIT 5 OFFSET 5;", dt);
        c("SELECT bi FROM tdata ORDER BY bi NULLS FIRST LIMIT 5 OFFSET 5;", "SELECT bi FROM tdata ORDER BY bi LIMIT 5 OFFSET 5;", dt);
        c("SELECT n FROM tdata ORDER BY n NULLS FIRST LIMIT 5 OFFSET 5;", "SELECT n FROM tdata ORDER BY n LIMIT 5 OFFSET 5;", dt);
        c("SELECT f FROM tdata ORDER BY f NULLS FIRST LIMIT 5 OFFSET 5;", "SELECT f FROM tdata ORDER BY f LIMIT 5 OFFSET 5;", dt);
        c("SELECT tt FROM tdata ORDER BY tt NULLS FIRST LIMIT 5 OFFSET 5;", "SELECT tt FROM tdata ORDER BY tt LIMIT 5 OFFSET 5;", dt);
        c("SELECT ts FROM tdata ORDER BY ts NULLS FIRST LIMIT 5 OFFSET 5;", "SELECT ts FROM tdata ORDER BY ts LIMIT 5 OFFSET 5;", dt);
        c("SELECT d FROM tdata ORDER BY d NULLS FIRST LIMIT 5 OFFSET 5;", "SELECT d FROM tdata ORDER BY d LIMIT 5 OFFSET 5;", dt);
    }
}   
...
TEST(Select, DISABLED_TopK_LIMIT_OFFSET_BottomHalf_DescendSort) {
    for (auto dt : {ExecutorDeviceType::CPU, ExecutorDeviceType::GPU}) {
    SKIP_NO_GPU();
        c("SELECT i FROM tdata ORDER BY i DESC NULLS LAST LIMIT 5 OFFSET 5;", "SELECT i FROM tdata ORDER BY i DESC LIMIT 5 OFFSET 5;", dt);
        c("SELECT b FROM tdata ORDER BY b DESC NULLS LAST LIMIT 5 OFFSET 5;", "SELECT b FROM tdata ORDER BY b DESC LIMIT 5 OFFSET 5;", dt);
        c("SELECT bi FROM tdata ORDER BY bi DESC NULLS LAST LIMIT 5 OFFSET 5;", "SELECT bi FROM tdata ORDER BY bi DESC LIMIT 5 OFFSET 5;", dt);
        c("SELECT n FROM tdata ORDER BY n DESC NULLS LAST LIMIT 5 OFFSET 5;", "SELECT n FROM tdata ORDER BY n DESC LIMIT 5 OFFSET 5;", dt);
        c("SELECT f FROM tdata ORDER BY f DESC NULLS LAST LIMIT 5 OFFSET 5;", "SELECT f FROM tdata ORDER BY f DESC LIMIT 5 OFFSET 5;", dt);
        c("SELECT tt FROM tdata ORDER BY tt DESC NULLS LAST LIMIT 5 OFFSET 5;", "SELECT tt FROM tdata ORDER BY tt DESC LIMIT 5 OFFSET 5;", dt);
        c("SELECT ts FROM tdata ORDER BY ts DESC NULLS LAST LIMIT 5 OFFSET 5;", "SELECT ts FROM tdata ORDER BY ts DESC LIMIT 5 OFFSET 5;", dt);
        c("SELECT d FROM tdata ORDER BY d DESC NULLS LAST LIMIT 5 OFFSET 5;", "SELECT d FROM tdata ORDER BY d DESC LIMIT 5 OFFSET 5;", dt);
    }
}
...
TEST(Select, DISABLED_TopK_LIMIT_OFFSET_GreaterThan_TotalOfDataRows_AscendSort) {
    for (auto dt : {ExecutorDeviceType::CPU, ExecutorDeviceType::GPU}) {
    SKIP_NO_GPU();
        c("SELECT i FROM tdata ORDER BY i NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT i FROM tdata ORDER BY i LIMIT 5 OFFSET 11;", dt);
        c("SELECT b FROM tdata ORDER BY b NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT b FROM tdata ORDER BY b LIMIT 5 OFFSET 11;", dt);
        c("SELECT bi FROM tdata ORDER BY bi NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT bi FROM tdata ORDER BY bi LIMIT 5 OFFSET 11;", dt);
        c("SELECT n FROM tdata ORDER BY n NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT n FROM tdata ORDER BY n LIMIT 5 OFFSET 11;", dt);
        c("SELECT f FROM tdata ORDER BY f NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT f FROM tdata ORDER BY f LIMIT 5 OFFSET 11;", dt);
        c("SELECT tt FROM tdata ORDER BY tt NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT tt FROM tdata ORDER BY tt LIMIT 5 OFFSET 11;", dt);
        c("SELECT ts FROM tdata ORDER BY ts NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT ts FROM tdata ORDER BY ts LIMIT 5 OFFSET 11;", dt);
        c("SELECT d FROM tdata ORDER BY d NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT d FROM tdata ORDER BY d LIMIT 5 OFFSET 11;", dt);
    }
}
...
TEST(Select, DISABLED_TopK_LIMIT_OFFSET_GreaterThan_TotalOfDataRows_DescendSort) {
    for (auto dt : {ExecutorDeviceType::CPU, ExecutorDeviceType::GPU}) {
    SKIP_NO_GPU();
        c("SELECT i FROM tdata ORDER BY i DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT i FROM tdata ORDER BY i DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT b FROM tdata ORDER BY b DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT b FROM tdata ORDER BY b DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT bi FROM tdata ORDER BY bi DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT bi FROM tdata ORDER BY bi DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT n FROM tdata ORDER BY n DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT n FROM tdata ORDER BY n DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT f FROM tdata ORDER BY f DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT f FROM tdata ORDER BY f DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT tt FROM tdata ORDER BY tt DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT tt FROM tdata ORDER BY tt DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT ts FROM tdata ORDER BY ts DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT ts FROM tdata ORDER BY ts DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT d FROM tdata ORDER BY d DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT d FROM tdata ORDER BY d DESC LIMIT 5 OFFSET 11;", dt);
    }
}
...
TEST(Select, DISABLED_TopK_LIMIT_OFFSET_DifferentOrders) {
    for (auto dt : {ExecutorDeviceType::CPU, ExecutorDeviceType::GPU}) {
    SKIP_NO_GPU();
        c("SELECT i,d FROM tdata ORDER BY d DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT i,d FROM tdata ORDER BY d DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT i,d FROM tdata ORDER BY i DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT i,d FROM tdata ORDER BY i DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT i,d FROM tdata ORDER BY i,d DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT i,d FROM tdata ORDER BY i,d DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT d FROM tdata ORDER BY i DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT d FROM tdata ORDER BY i DESC LIMIT 5 OFFSET 11;", dt);
        c("SELECT d FROM tdata ORDER BY i,d DESC NULLS LAST LIMIT 5 OFFSET 11;", "SELECT d FROM tdata ORDER BY i,d DESC LIMIT 5 OFFSET 11;", dt);

        c("SELECT i,d FROM tdata ORDER BY d NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT i,d FROM tdata ORDER BY d LIMIT 5 OFFSET 11;", dt);
        c("SELECT i,d FROM tdata ORDER BY i NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT i,d FROM tdata ORDER BY i LIMIT 5 OFFSET 11;", dt);
        c("SELECT i,d FROM tdata ORDER BY i,d NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT i,d FROM tdata ORDER BY i,d LIMIT 5 OFFSET 11;", dt);
        c("SELECT d FROM tdata ORDER BY i NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT d FROM tdata ORDER BY i LIMIT 5 OFFSET 11;", dt);
        c("SELECT d FROM tdata ORDER BY i,d NULLS FIRST LIMIT 5 OFFSET 11;", "SELECT d FROM tdata ORDER BY i,d LIMIT 5 OFFSET 11;", dt);
    }
}
