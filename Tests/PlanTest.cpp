...
TEST(ParseAnalyzePlan, Views) {
  ...
  EXPECT_THROW(run_ddl("create materialized view if not exists moo as select * from skinny where a > 15;");, std::runtime_error);
  EXPECT_THROW(run_ddl(
                      "create materialized view if not exists goo with (storage = 'gpu', refresh = "
                      "'auto') as select * from skinny where a > 15;");, std::runtime_error);
  EXPECT_THROW(run_ddl(
                      "create materialized view if not exists mic (col, avg_b) with (storage = 'mic', refresh "
                      "= 'manual') as select c, avg(b) from skinny where a > 10 group by c;");, std::runtime_error);
  EXPECT_THROW(run_ddl(
                      ...
                      "not null or k <= 100000000000 and c = 'xyz';");, std::runtime_error);
