...
class S3SelectQueryTest : public SelectQueryTest {
 protected:
  void SetUp() override {
    SelectQueryTest::SetUp();
    sql("DROP SERVER IF EXISTS s3_server;");
    sql("CREATE SERVER s3_server FOREIGN DATA WRAPPER omnisci_csv "s +
        "WITH (storage_type = 'AWS_S3', s3_bucket = 'mapd-parquet-testdata', "
        "AWS_REGION = 'us-east-1');");
    // Currently a s3_archive is instantiated in MapDServer to initialize the S3 API
    // This should be done in a cleaner way, but need to investigate how this impacts the
    // Arrow API
    static auto s3_survivor =
        std::make_unique<S3Archive>("s3://omnisci/s3_survivor.txt", true);
  }
  ...
  inline static const std::string trip_schema = R"(
...
TEST_F(S3SelectQueryTest, SingleFile) {
  std::string query =
      "CREATE FOREIGN TABLE test_foreign_table ("s + trip_schema +
      " ) SERVER s3_server WITH (file_path = 'trip.compressed/trip_data_9.csv');";
  sql(query);
  TQueryResult result;
  sql(result, "SELECT COUNT(*), AVG(trip_distance) FROM test_foreign_table;");
  assertResultSetEqual({{i(100), 1.0}}, result);
}

TEST_F(S3SelectQueryTest, MultiFile) {
  std::string query = "CREATE FOREIGN TABLE test_foreign_table ("s + trip_schema +
                      " ) SERVER s3_server WITH (file_path = 'trip.compressed/');";
  sql(query);
  TQueryResult result;
  sql(result, "SELECT COUNT(*), AVG(trip_distance) FROM test_foreign_table;");
  assertResultSetEqual({{i(1000), 1.0}}, result);
}

TEST_F(S3SelectQueryTest, FileMissing) {
  std::string query =
      "CREATE FOREIGN TABLE test_foreign_table ("s + trip_schema +
      " ) SERVER s3_server WITH (file_path = 'trip.compressed/trip_data_missing.csv');";
  sql(query);
  queryAndAssertException(
      "SELECT * FROM test_foreign_table;",
      "Exception: No object was found with s3 url "
      "'s3://mapd-parquet-testdata/trip.compressed/trip_data_missing.csv'");
}

TEST_F(S3SelectQueryTest, InvalidBucket) {
  sql("CREATE SERVER s3_server_bucket FOREIGN DATA WRAPPER omnisci_csv "s +
      "WITH (storage_type = 'AWS_S3', s3_bucket = 'mapd-invalid-bucket', "
      "AWS_REGION = 'us-east-1');");
  std::string query =
      "CREATE FOREIGN TABLE test_foreign_table ("s + trip_schema +
      " ) SERVER s3_server_bucket WITH (file_path = 'trip.compressed/trip_data_9.csv');";
  sql(query);
  queryAndAssertException("SELECT * FROM test_foreign_table;",
                          "Exception: Could not list objects for s3 url "
                          "'s3://mapd-invalid-bucket/trip.compressed/trip_data_9.csv'");
  sql("DROP FOREIGN TABLE test_foreign_table;");
  sql("DROP SERVER s3_server_bucket;");
}
