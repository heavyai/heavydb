{
  "baseTemplate": "Marks/lineTemplate.geom",
  "operators": [
    {
      "name": "kReplaceFirstTag",
      "string1": "GeometryShaderInputs",
      "string2": "\n  layout (location = 0)  in uint64_t gRowId[];\n  layout (location = 1)  in float gOpacity[];\n  layout (location = 2)  in vec4 gColor[];\n  layout (location = 3)  in float gStrokeWidth[];\n  layout (location = 4)  in float gStrokeOpacity[];\n  layout (location = 5)  in int gLineJoin[];\n  layout (location = 6)  in float gMiterLimit[];",
      "required": false
    },
    {
      "name": "kReplaceFirstTag",
