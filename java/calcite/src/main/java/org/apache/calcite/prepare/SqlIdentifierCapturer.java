  ...
  public void scan(SqlNode root) {
    ...
    if (root instanceof SqlBasicCall) {
      ...
      if (call.getOperator().getKind() == SqlKind.ARGUMENT_ASSIGNMENT) {
        ...
        if (call.getOperandList().get(0).getKind() == SqlKind.CURSOR) {
          SqlBasicCall cursor_call = (SqlBasicCall) call.getOperandList().get(0);
          if (cursor_call.operandCount() == 0) {
            return;
          }
          scan(cursor_call.getOperandList().get(0));
          return;
        } else {
          return;
        }
        ...
        scan(call.getOperandList().get(0));
        return;
      }
    }
