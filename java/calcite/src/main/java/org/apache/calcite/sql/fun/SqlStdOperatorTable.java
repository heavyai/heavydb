 ...
 *
 * This file is a modified derivative of Apache Calcite's org.apache.calcite.sql.fun.SqlStdOperatorTable.
 *
...
public class SqlStdOperatorTable extends ReflectiveSqlOperatorTable {
  ...
  private static final SqlOperandTypeChecker SET_OP_OPERAND_TYPE_CHECKER =
      new SqlOperandTypeChecker() {
        @Override
        public boolean checkOperandTypes(
            SqlCallBinding callBinding, boolean throwOnFailure) {
          if (hasMismatchedSetOpTypedNullIntegerCast(callBinding)) {
            if (throwOnFailure) {
              throw callBinding.newValidationSignatureError();
            }
            return false;
          }
          return OperandTypes.SET_OP.checkOperandTypes(callBinding, throwOnFailure);
        }

        @Override
        public SqlOperandCountRange getOperandCountRange() {
          return OperandTypes.SET_OP.getOperandCountRange();
        }

        @Override
        public String getAllowedSignatures(SqlOperator op, String opName) {
          return OperandTypes.SET_OP.getAllowedSignatures(op, opName);
        }
      };

  public static final SqlSetOperator UNION =
      new SqlSetOperator("UNION", SqlKind.UNION, 12, false,
          ReturnTypes.LEAST_RESTRICTIVE, null, SET_OP_OPERAND_TYPE_CHECKER);
  ...
  private static boolean hasMismatchedSetOpTypedNullIntegerCast(
      SqlCallBinding binding) {
    final SqlCall call = binding.getCall();
    if (!isSetOp(call.getKind()) || binding.getOperandCount() != 2) {
      return false;
    }

    final RelDataType leftType = binding.getOperandType(0);
    final RelDataType rightType = binding.getOperandType(1);
    if (!leftType.isStruct() || !rightType.isStruct()
        || leftType.getFieldCount() != rightType.getFieldCount()) {
      return false;
    }

    final SqlNode leftNode = binding.operand(0);
    final SqlNode rightNode = binding.operand(1);
    final SqlValidator validator = binding.getValidator();
    for (int fieldIndex = 0; fieldIndex < leftType.getFieldCount(); fieldIndex++) {
      final SqlTypeName leftCastType =
          getTypedNullIntegerCastType(leftNode, fieldIndex, validator);
      final SqlTypeName rightCastType =
          getTypedNullIntegerCastType(rightNode, fieldIndex, validator);
      if (leftCastType != null && rightCastType != null
          && leftCastType != rightCastType) {
        return true;
      }

      final SqlTypeName leftFieldType =
          leftType.getFieldList().get(fieldIndex).getType().getSqlTypeName();
      final SqlTypeName rightFieldType =
          rightType.getFieldList().get(fieldIndex).getType().getSqlTypeName();
      if (leftCastType != null && SqlTypeName.INT_TYPES.contains(rightFieldType)
          && leftCastType != rightFieldType) {
        return true;
      }
      if (rightCastType != null && SqlTypeName.INT_TYPES.contains(leftFieldType)
          && rightCastType != leftFieldType) {
        return true;
      }
    }
    return false;
  ...
  private static boolean isSetOp(SqlKind kind) {
    return kind == SqlKind.UNION || kind == SqlKind.INTERSECT
        || kind == SqlKind.EXCEPT;
  }

  private static SqlTypeName getTypedNullIntegerCastType(
      SqlNode query, int fieldIndex, SqlValidator validator) {
    final SqlNode fieldNode = getSetOpFieldNode(query, fieldIndex);
    if (fieldNode == null) {
      return null;
    }

    final SqlNode expression = stripAs(fieldNode);
    if (expression.getKind() != SqlKind.CAST) {
      return null;
    }

    final SqlCall castCall = (SqlCall) expression;
    if (!SqlUtil.isNullLiteral(castCall.operand(0), false)
        || !(castCall.operand(1) instanceof SqlDataTypeSpec)) {
      return null;
    }

    final RelDataType castType =
        ((SqlDataTypeSpec) castCall.operand(1)).deriveType(validator);
    final SqlTypeName castTypeName = castType.getSqlTypeName();
    return SqlTypeName.INT_TYPES.contains(castTypeName) ? castTypeName : null;
  }

  private static SqlNode getSetOpFieldNode(SqlNode query, int fieldIndex) {
    if (query.getKind() != SqlKind.SELECT) {
      return null;
    }

    final SqlNodeList selectList = ((SqlSelect) query).getSelectList();
    if (selectList == null || fieldIndex < 0 || fieldIndex >= selectList.size()) {
      return null;
    }
    return selectList.get(fieldIndex);
  }

  private static SqlNode stripAs(SqlNode node) {
    SqlNode result = node;
    while (result.getKind() == SqlKind.AS) {
      result = ((SqlCall) result).operand(0);
    }
    return result;
  }
