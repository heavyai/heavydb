 ...
 *
 * This file is a modified derivative of Apache Calcite's org.apache.calcite.sql2rel.SqlToRelConverter.
 *
...
import com.mapd.calcite.parser.HeavyDBSqlOperatorTable.ExtTableFunction; // HEAVY.AI
...
import java.util.function.BiPredicate; // HEAVY.AI
...
public class SqlToRelConverter {
  ...
  private void substituteSubQuery(Blackboard bb, SubQuery subQuery) {
    ...
    // HEAVY.AI START BLOCK
    BiPredicate<SqlNode, SqlNode> biPredicate =((HeavyDBSqlToRelConverter.Config) config).getExpandPredicate();
    boolean isExpand = biPredicate.test(bb.getTopNode(), subQuery.node);
    // HEAVY.AI END BLOCK
  ...
  // HEAVY.AI: changed from private static to protected to allow
  // HeavyDBSqlToRelConverter to override and handle ExtTableFunction column mappings.
  ...
  protected RelNode createAggregate(Blackboard bb, ImmutableBitSet groupSet,
    ...
    // HEAVY.AI START BLOCK: rewrote to use LogicalAggregate directly (avoiding RelBuilder)
    // so that queries like "SELECT count(*) FROM test LEFT JOIN test_inner ON test.x = test_inner.x WHERE test.y > 42"
    // produce the same relational algebra as the 1.25 version of calcite. Calling LogicalAggregate
    // directly ensures an intermediate LocalProject node is not removed via
    // pruneAggregateInputFieldsAndDeduplicateAggCalls. LogicalAggregate also requires a sorted groupSet.
    // Original Calcite code:
    ImmutableList<ImmutableBitSet> sortedGroupSets =
        ImmutableBitSet.ORDERING.immutableSortedCopy(groupSets);
    return LogicalAggregate.create(bb.root(), ImmutableList.of(), groupSet, sortedGroupSets, aggCalls);
    // HEAVY.AI END BLOCK
  }
  ...
  protected class Blackboard implements SqlRexContext, SqlVisitor<RexNode>,
    ...
    @Override public RexNode convertExpression(SqlNode expr) {
      ...
      // HEAVY.AI START BLOCK
      BiPredicate<SqlNode, SqlNode> biPredicate =((HeavyDBSqlToRelConverter.Config) config).getExpandPredicate();
      boolean isExpand = biPredicate.test(getTopNode(), expr);
      if (!isExpand) {
        final SqlCall call;
        final SqlNode query;
        final RelRoot root;
    ...
    @Override public RexNode visit(SqlCall call) {
      ...
      // HEAVY.AI START BLOCK
      // Calcite 1.41's SqlCallBinding only pads omitted arguments with DEFAULT
      // when the operand metadata reports isFixedParameters()==true; our UDTFs
      // report false to keep SQL:2003 routine resolution from running canCastFrom()
      // on CURSOR-typed params. For HeavyDB UDTFs, use the validated operator and
      // the HeavyDB-specific permutation/default rewrite so CURSOR ColumnList fields
      // stay grouped for backend binding.
      SqlCall permutedCall =
          new SqlCallBinding(validator(), scope, call).permutedCall();
      if (call.getOperator() instanceof ExtTableFunction) {
        ExtTableFunction tf = (ExtTableFunction) call.getOperator();
        if (tf.supportsDefaultArguments()) {
          permutedCall = tf.rewriteCallWithDefaultArguments(call);
        } else {
          permutedCall = tf.permuteOperandsWithDefaultMarkers(call);
        }
      }
