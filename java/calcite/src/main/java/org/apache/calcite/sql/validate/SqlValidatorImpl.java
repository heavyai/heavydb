...
//
// Clang formatting make it difficult to compare with original Calcite file
//   turn it off
// clang-format off
//
// Changes for HEAVY.AI in this file are bracketed by the string "HEAVY.AI"
//
...
//
// These fail because
//    The function LTRIM requires 2 arguments, and the other has a default
//      -> thus allowing one "LTRIM(name)" for "LTRIM(' ', 'name')
//    The expansion of the select-list ("TRIM(name)" cannot be done first
//      -> because of timing issues (see comments in pre-existing code)
//    The substitution of "1" and "n" in the group-by is then done first
//      with the string "LTRIM(name)"
//    Once substituted, the code returned, assuming no other expansion was needed
//      -> which was incorrect
//
// FIX: expand the replacement expr before doing the replacement
...
public class SqlValidatorImpl implements SqlValidatorWithHints {
  ...
  public SqlNode expandGroupByOrHavingExpr(SqlNode expr,
    ...
    if (expr != newExpr) {
      // HEAVY.AI modification - begin
      // This 'if' condition was added to expand the replacement expression
      //   because it might contain a UDF that has missing/defaulted arguments
      //   that have not yet been substituted
      SqlNode nestedNewExpr = newExpr.accept(expander);
      if (newExpr != nestedNewExpr) {
        newExpr = nestedNewExpr;
      }
      // HEAVY.AI modification - end
      setOriginal(newExpr, expr);
    }
