 ...
 *
 * This file is a modified derivative of Apache Calcite's org.apache.calcite.sql.type.SqlTypeFactoryImpl.
 *
  ...
  private @Nullable RelDataType leastRestrictiveSqlType(List<RelDataType> types) {
    ...
    for (int i = 0; i < types.size(); ++i) {
      ...
      if (SqlTypeUtil.inCharOrBinaryFamilies(type)) {
        ...
        // HeavyAI: the block below is disabled because it widens a TIMESTAMP(n) to
        // TIMESTAMP(n+m) when the two sides of a comparison have different precisions.
        // HeavyDB instead requires the higher-precision side to be cast down to the
        // lower precision, so allowing Calcite to silently promote the result type here
