...
public class TablePermissionsTest {
  ...
  public void testTablePermissions() throws Exception {
    ...
    shouldThrowException("foo not allowed to select", () -> foo.runSql("SELECT * from bill_table"));

    bill.runSql("GRANT SELECT ON TABLE bill_table TO salesDept"); // foo
    bob.runSql("SELECT * from bill_table");
    foo.runSql("SELECT * from bill_table");

    shouldThrowException("insert not allowed", () -> bob.runSql("INSERT INTO bill_table VALUES(1)"));
    shouldThrowException("insert not allowed ", () -> foo.runSql("INSERT INTO bill_table VALUES(1)"));
    ...
    shouldThrowException("update not allowed", () -> bob.runSql("UPDATE bill_table SET id = 2 WHERE id = 0"));
    shouldThrowException("update not allowed ", () -> foo.runSql("UPDATE bill_table SET id = 2 WHERE id = 0"));
    ...
    shouldThrowException("update not allowed", () -> bob.runSql("DELETE FROM bill_table WHERE id = 0"));
    shouldThrowException("update not allowed ", () -> foo.runSql("DELETE FROM bill_table WHERE id = 0"));
