...
public class CatalogConcurrencyTest {
  ...
  private void run_test(MapdTestClient dba, MapdTestClient user, String prefix, int max) throws Exception {
  ...
  private void runTest(String db, String dbaUser, String dbaPassword, String dbUser, String dbPassword) throws Exception {
    int num_threads = 5;
    final int runs = 25;
    ...
    for (int i=0; i<num_threads; i++) {
      ...
      Thread t = new Thread(new Runnable() {
        ...
        public void run() {
          try {
            MapdTestClient dba = MapdTestClient.getClient("localhost", 9091, db, dbaUser, dbaPassword);
            MapdTestClient user = MapdTestClient.getClient("localhost", 9091, db, dbUser, dbPassword);
