...
Data on disk is organized into metadata pages and data multipages. The `BufferMgr` class manages data in each level of the memory hierarchy, with data on disk considered the "lowest" level. Specifically, the `FileMgr` manages loading data from disk and flushing data back to disk during inserts, updates, and deletes. Initially, a single `GlobalFileMgr` is created to serve as the entry point for all file management. In turn, the `GlobalFileMgr` has a child file manager for each table in the current database (see diagram :ref:`file-manager-structure`).
...
Foreign Tables and Disk Cache
=============================

The ``GlobalFileMgr`` hierarchy above applies to **native** tables stored under the data
directory. **Foreign tables** do not use per-table ``data/table_<db>_<id>`` folders;
instead, ``PersistentStorageMgr`` routes their chunk keys to ``ForeignStorageMgr`` and
a per-table ``ForeignDataWrapper`` that reads from files, ODBC, or other sources.

When disk cache is enabled (default level ``foreign_tables`` for FSI), ``CachingFileMgr``
under ``{data_dir}/disk_cache`` can persist foreign (and optionally native mutable)
chunk data separately from native multipage files. See :doc:`../foreign_storage/index`
and :doc:`../runtime/index`.
