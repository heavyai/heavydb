...
struct CopyParams {
  ...
  CopyParams() : delimiter(','), null_str("\\N"), has_header(true), quoted(true), quote('"'), escape('"'), line_delim('\n'), array_delim(','), array_begin('{'), array_end('}'), threads(0) {}
};
...
class Loader {
  public:
    Loader(const Catalog_Namespace::Catalog &c, const TableDescriptor *t) : catalog(c), table_desc(t), column_descs(c.getAllColumnMetadataForTable(t->tableId,false)) { init(); };
