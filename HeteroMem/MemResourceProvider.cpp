...
MemoryResourceProvider::MemoryResourceProvider()
    : mem_resources_(3, std::pmr::get_default_resource()) {}
    ...
    while (!pmem_dirs_file.eof()) {
      ...
      if (!line.empty()) {
        ...
        if (pmem_mem_resource_.get()) {
          LOG(FATAL) << "For now OmniSciDB does not support more than one directory for "
                        "PMM volatile"
                     << std::endl;
          return;
        }
        ...
        pmem_mem_resource_ =
            std::make_unique<pmem_memory_resource_type>(path, size * 1024 * 1024 * 1024);
