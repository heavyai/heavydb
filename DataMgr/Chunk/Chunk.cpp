...
std::shared_ptr<Chunk> Chunk::getChunk(const ColumnDescriptor* cd,
  ...
  if ((memoryLevel == CPU_LEVEL) && (cd->isHotCol != true) &&
      (cd->isSoftHotCol != true) && data_mgr->pmmStorePresent() &&
  ...
  chunkp->getChunkBuffer(data_mgr, key, level, deviceId, query_id, numBytes, numElems);
  return chunkp;
}
#else /* HAVE_DCPMM */
...
#ifdef HAVE_DCPMM
    buffer_ =
        data_mgr->getChunkBuffer(bd, subKey, mem_level, query_id, device_id, num_bytes);
#else  /* HAVE_DCPMM */
    buffer_ = data_mgr->getChunkBuffer(bd, subKey, mem_level, device_id, num_bytes);
#endif /* HAVE_DCPMM */
    subKey.pop_back();
...
#ifdef HAVE_DCPMM
    buffer_ =
        data_mgr->getChunkBuffer(bd, key, mem_level, query_id, device_id, num_bytes);
#else  /* HAVE_DCPMM */
    buffer_ = data_mgr->getChunkBuffer(bd, key, mem_level, device_id, num_bytes);
#endif /* HAVE_DCPMM */
  }
}
...
void Chunk::createChunkBuffer(DataMgr* data_mgr,
  ...
  if (column_desc_->columnType.is_varlen() &&
    ...
    buffer_ = data_mgr->createChunkBuffer(
        bd, subKey, mem_level, maxRows, sqlTypeSize, device_id, page_size);
#else  /* HAVE_DCPMM */
    buffer_ = data_mgr->createChunkBuffer(bd, subKey, mem_level, device_id, page_size);
#endif /* HAVE_DCPMM */
    subKey.pop_back();
    subKey.push_back(2);  // 2 for the index buffer_
    bd.isIndex = true;
    ...
    index_buf_ = data_mgr->createChunkBuffer(
        bd, subKey, mem_level, maxRows, sqlTypeSize, device_id, page_size);
#else  /* HAVE_DCPMM */
    index_buf_ = data_mgr->createChunkBuffer(bd, subKey, mem_level, device_id, page_size);
#endif /* HAVE_DCPMM */
  } else {
#ifdef HAVE_DCPMM
    buffer_ = data_mgr->createChunkBuffer(
        bd, key, mem_level, maxRows, sqlTypeSize, device_id, page_size);
#else  /* HAVE_DCPMM */
    buffer_ = data_mgr->createChunkBuffer(bd, key, mem_level, device_id, page_size);
#endif /* HAVE_DCPMM */
  }
}
