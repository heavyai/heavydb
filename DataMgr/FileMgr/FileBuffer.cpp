...
void FileBuffer::readMetadata(void)
{
  ...
  if (has_encoder) {
    ...
    sql_type.set_compression(static_cast<EncodingType>(pmmBufferDescriptor_->metaData[7]));
...
#ifdef HAVE_DCPMM
void
FileBuffer::constructPersistentBuffer(int8_t *addr, PersistentBufferDescriptor *p)
{
  pmmMem_ = addr;
  pmmBufferDescriptor_ = p;
}
