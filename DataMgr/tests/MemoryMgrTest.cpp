...
namespace Memory_Namespace {
    ...
    TEST_F (MemoryMgrTest, deleteChunk) {
        ...
        AbstractBuffer * cpuChunk = memoryMgr -> createChunk(CPU_LEVEL,key);
        const int numInts = 10000;
        int * data1 = new int [numInts];
        for (size_t i = 0; i < numInts; ++i) {
            data1[i] = i;
        }
        cpuChunk -> append((mapd_addr_t)data1,numInts*sizeof(int),Memory_Namespace::CPU_BUFFER);
        memoryMgr -> checkpoint();
    ...
    TEST_F (MemoryMgrTest, buffer) {
        AbstractBuffer *cpuBuffer = memoryMgr -> createBuffer(CPU_LEVEL,0,4096);
        const int numInts = 10000;
        int * data1 = new int [numInts];
        for (size_t i = 0; i < numInts; ++i) {
            data1[i] = i;
        ...
        EXPECT_EQ(memoryMgr -> bufferMgrs_[1][0] -> getNumChunks(),1);
        EXPECT_EQ(memoryMgr -> bufferMgrs_[0][0] -> getNumChunks(),0);
    }
}
