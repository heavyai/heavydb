...
- `-DENABLE_LLVM_DBG=off` - Switch to local debug version of llvm. Default is `off`.
- `-DENABLE_MULTIFRAG_JOIN=on` - Enable Join on Multi-frag table. Default is `on`.
- `-DENABLE_ODBC=off` - Build ODBC driver. Default is `off`.
...
- `-DENABLE_RENDERING=off` - Build backend renderer. Default is `off`.
