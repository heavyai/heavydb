...
# Build config: ASAN + RelWithDebInfo, CPU-only static. Source of truth for
# build-asan in .github/workflows/nightly.yml. Pairs with the asan (ctest)
# and columnar-asan (gtest) test variants.
#
# Locally, append `-DHEAVYAI_PARTITION=N` (N in 1..4) to CMAKE_FLAGS to match
# a specific CI partition; omit it for an unpartitioned build.
...
export IMAGE_NAME="pr-build/asan"
