...
const Parser::SharedDictionaryDef compress_reference_path(
  ...
  for (; idx < shared_dict_defs.size(); idx++) {
    if (!shared_dict_defs[idx].get_column().compare(cur_node.get_column()) &&
        !shared_dict_defs[idx].get_foreign_table().compare(
            cur_node.get_foreign_table()) &&
        !shared_dict_defs[idx].get_foreign_column().compare(
            cur_node.get_foreign_column())) {
      break;
    }
  }
  ...
  for (size_t j = 0; j < shared_dict_defs.size(); j++) {
    for (size_t i = 0; i < shared_dict_defs.size(); ++i) {
      if (!shared_dict_defs[i].get_column().compare(
              shared_dict_defs[ret_val_idx].get_foreign_column())) {
        ret_val_idx = i;
        break;
      }
    }
    if (shared_dict_defs[ret_val_idx].get_foreign_table().compare(
            cur_node.get_foreign_table())) {
      // found a dictionary share definition which shares the dict outside this table to
      // be created
      break;
    }
  }
...
void validate_shared_dictionary_order(
  ...
  if (!shared_dict_def->get_column().compare(shared_dict_def->get_foreign_column())) {
    throw std::runtime_error(
        "Dictionary cannot be shared with itself. For dictionary : " +
        reference_col_qualified_name);
  }
  auto table_name = stmt->get_table();
  CHECK(!shared_dict_def->get_foreign_table().compare(*table_name));
  auto col = std::find_if(
      columns.rbegin(), columns.rend(), [shared_dict_def](const ColumnDescriptor& elem) {
        return !elem.columnName.compare(shared_dict_def->get_column());
      });
  CHECK(col != columns.rend());
  auto ref_col =
      std::find_if(col, columns.rend(), [shared_dict_def](const ColumnDescriptor& elem) {
        return !elem.columnName.compare(shared_dict_def->get_foreign_column());
      });

  if (ref_col == columns.rend()) {
    throw std::runtime_error("Dictionary dependencies might create a cycle for " +
                             shared_dict_def->get_column() + "referencing " +
                             reference_col_qualified_name);
  }
}

namespace {
const ColumnDescriptor* lookup_column(const std::string& name,
                                      const std::list<ColumnDescriptor>& columns) {
...
void validate_shared_dictionary(
  ...
  if (!cd_ptr->columnType.is_string() ||
      cd_ptr->columnType.get_compression() != kENCODING_DICT) {
    throw std::runtime_error("Column " + col_qualified_name +
                             " must be a dictionary encoded string");
  }
  const std::list<std::unique_ptr<Parser::TableElement>>& table_element_list =
      stmt->get_table_element_list();
  ...
  if (foreign_td) {
    ...
    const auto reference_col_qualified_name =
        *table_name + "." + shared_dict_def->get_foreign_column();
    const auto reference_cd_ptr =
        lookup_column(shared_dict_def->get_foreign_column(), columns);
    if (!reference_cd_ptr) {
    ...
    validate_shared_dictionary_order(
        stmt, shared_dict_def, shared_dict_defs_so_far, columns);
  }
  const auto it =
      std::find_if(shared_dict_defs_so_far.begin(),
  ...
  if (it != shared_dict_defs_so_far.end()) {
    throw std::runtime_error("Duplicate shared dictionary hint for column " +
                             *table_name + "." + shared_dict_def->get_column());
  }
}
