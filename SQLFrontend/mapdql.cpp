...
void process_backslash_commands(char* command, ClientContext& context) {
  using Params = CommandResolutionChain<>::CommandTokenList const&;
  using ParamTypeRef = std::remove_reference<Params>::type::value_type const&;

  // FIX-ME:  Refactor these into functors.  See CommandFunctor.h
  auto h_command = [&](Params p) {
  ...
  auto d_command = [&](Params p) {
    ParamTypeRef table_name(p[1]);

    if (!thrift_with_retry(kGET_TABLE_DETAILS, context, table_name.c_str())) {
      return;
    }
  ...
  auto c_command = [&](Params p) {
    ParamTypeRef db(p[1]);
    ParamTypeRef user(p[2]);
    ParamTypeRef passwd(p[3]);
  ...
  // clang-format off
  auto resolution_status = CommandResolutionChain<>( command, "\\h", 1, h_command )
  ( "\\d", 2, d_command, "Usage: \\d <table>" )
  ( "\\o", 2, o_command, "Usage: \\o <table>" )
  ( "\\t", 1, ListTablesCmd<>( context ) )
  ( "\\v", 1, ListViewsCmd<>( context ) )
  ( "\\c", 4, c_command, "Usage: \\c <database> <user> <password>." )
