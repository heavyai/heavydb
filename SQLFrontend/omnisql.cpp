...
void set_column_cold(ClientContext& context, char const *table, char const *column) {
  if (thrift_with_retry(kCOOL_COLUMN, context, table, column)) {
    std::cout << "Column " << column << " in table " << table << " is set to cold." << std::endl;
  }
}
...
void estimate_dram_size(ClientContext& context, char const *perf_bar_str) {
  ...
  if (size == 0) {
    ...
    if (size > 1024 * 1024 * 1024) {
      std::cout << ((size * 1.0) / (1024 * 1024 * 1024)) << "GB of DRAM recommended." << std::endl;
    }
    else {
      if (size > 1024 * 1024) {
        std::cout << ((size * 1.0) / (1024 * 1024)) << "MB of DRAM recommended." << std::endl;
      }
      else {
